...
class InputFilter {
    ...
    public $tagsMethod = 0;  // default = 0
    public $attrMethod = 0;  // default = 0

    public $xssAuto = 1;     // default = 1
    ...
    function __construct($tagsArray = [], $attrArray = [], $tagsMethod = 0, $attrMethod = 0, $xssAuto = 1) {      
    ...
    function filterAttr($attrSet) {
        ...
        foreach ($attrSet as $attr) {
            if (!$attr) continue;
            $attrSubSet = explode('=', trim($attr));
            list($attrName) = explode(' ', $attrSubSet[0]);
            
            if ((!preg_match("/^[a-z][a-z0-9]*$/i", $attrName)) || 
                ((in_array(strtolower($attrName), $this->attrBlacklist)) || 
