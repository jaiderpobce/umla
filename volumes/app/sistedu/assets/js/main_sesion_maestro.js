$(document).ready(function(){
    ...
    //},
       { responsivePriority: 1, targets: 3,width: '5%'},
       { responsivePriority: 2, targets: 2,width: '10%' },
...
$("#formPersonas").submit(function(e){
    ...
    nom_tipo= $('select[name="ti_sesion"] option:selected').text();
    nom_entrenador= $('select[name="id_entrenador"] option:selected').text();
    console.log(nom_tipo);
    ti_sesion = $.trim($("#ti_sesion").val());   
    nombre = $.trim($("#nombre").val());
    descripcion = $.trim($("#descripcion").val());
    descripcion = 'NS';
    fecha = $.trim($("#fecha").val());
    id_entrenador = $.trim($("#id_entrenador").val());
    id_macrociclo = $.trim($("#id_macrociclo").val());
    id_macrociclo = '1';
    vol = ($.trim($("#vol").val())) ? $.trim($("#vol").val()) : 0;
    idurl= $("#idurl").val();

    console.log(opcion);
 if(ti_sesion==''|| nombre ==''||fecha==''||id_entrenador==''){
    ...
    $.ajax({
       // url: idurl+"bd/crud.php",
        ...
        type: "POST",
        dataType: "json",
        data: {id:id,ti_sesion:ti_sesion,nombre:nombre, descripcion:descripcion, fecha:fecha, id_entrenador:id_entrenador, id_macrociclo:id_macrociclo, vol:vol, opcion:opcion},
        success: function(data){  
            //console.log(data);
            id = data[0].id;   
            ti_sesion = data[0].id_tsesion;    
            nombre = data[0].nombre;
            descripcion = data[0].descripcion; 
            fecha= data[0].fecha;
            id_entrenador= data[0].id_entrenador;
            id_macrociclo= data[0].id_mc;
            vol= data[0].vol;
            if(opcion == 1){tablaPersonas.row.add([id,nom_tipo,nombre,fecha,nom_entrenador,vol]).draw();}
            else{tablaPersonas.row(fila).data([id,nom_tipo,nombre,fecha,nom_entrenador,vol]).draw();} 
            location.reload();
